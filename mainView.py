...
class connectDatabase():
    ...
    def setUniqateColumn(self,data):
        self._uniqateColumn.append(data)

    def checkUniqateColumn(self,name):
        for x in self._uniqateColumn:
            print(name+" == "+x)
            if x==name:
                return 1
    ...
    def findNextTable(self, name):
        print("called findNextTable")
        print(self._constarinTable)
        for x in self._constarinTable:   
            print(name+" == "+ x[0])
            if name == x[0]:
                return x
    ...
    def createInsertString(self):
        ...
        for y in self._dbData:
            ...
            for x in insertArraySchema:
                ...
                isUniqate=self.checkUniqateColumn(x[0])
                print(isUniqate)
                if fkData is not None:
                    insertString2 = "INSERT INTO "+x[0]+" ("+insertString +","+insertValues+") VALUES ('"+fkData[1]+"',(SELECT "+fkData[3]+" FROM "+fkData[2]+"order by "+fkData[1]+" dessc limit 1))"
                else:
                    insertString2 = "INSERT INTO "+x[0]+" ('"+insertValues+"') VALUES ('"+insertString+"')"
                # print(insertString2)
    ...
    def setDbConnection(self,_dbHostName="localhost" , _dbLogin="mat" , _dbPassowrd="zaq" , _dbDatabase="papierosy"):
    ...
    def getDataTables(self):
        ...
        # cursor.execute("SELECT distinct table_name as selected_table,(SELECT GROUP_CONCAT(column_name) from information_schema.columns where table_schema = 'papierosy' and COLUMN_KEY != 'PRI' and table_name=selected_table order by table_name,ordinal_position ) from information_schema.columns where table_schema = 'papierosy' order by table_name,ordinal_position")
        cursor.execute("SELECT distinct table_name as selected_table,(SELECT GROUP_CONCAT(column_name) from information_schema.columns where table_schema = 'papierosy'  and table_name=selected_table order by table_name,ordinal_position ) from information_schema.columns where table_schema = 'papierosy' order by table_name,ordinal_position")
        self.dbSchema = cursor.fetchall()
        cursor.execute("SELECT  table_name,  column_name, referenced_table_name, REFERENCED_COLUMN_NAME  FROM INFORMATION_SCHEMA.key_column_usage WHERE referenced_table_schema = 'papierosy'   AND referenced_table_name IS NOT NULL ORDER BY table_name, column_name")
        self._constarinTable = cursor.fetchall()

    def sendDatatoDB(self):
        print("function sendDatatoDB")
        config = {
            'user':     "mat",
            'password': "zaq",
            'host':     "localhost",
        ...
        for x in self._insertSchemaArray:
            print(x)
            cursor.execute(x)
            db.commit()
...
class FormWidget(QWidget):
    ...
    def fillData(self):
        ...
        if not (connection.dbSchema is None):
            ...
            if s.uniqateCell ==1 :
                self.tableDBChoose.item(self.tableDBChoose.currentRow(), self.tableDBChoose.currentColumn()).setBackground(QtGui.QColor(255,0,127))
                item=self.tableDBChoose.item(self.tableDBChoose.currentRow(), self.tableDBChoose.currentColumn()).text()
                connection.setUniqateColumn(item)
...
class mainView(QtGui.QMainWindow):
    ...
    def checkUniqateTables(self):
        print("called checkUniqateTables")
        print(self.form_widget.tableDBChoose.columnCount())
        col = self.form_widget.tableDBChoose.columnCount()
        tmpArray=[]
        for x in range(0,col):
            print(self.form_widget.tableDBChoose.item(0, x).backgroundColor)
        #     if columnName== self.form_widget.tableDBChoose.item(0, x).text():
        #         tmpArray.append()
        #     else: 
        #         return 0   

        # column.setUniqateColumn(tmpArray)
